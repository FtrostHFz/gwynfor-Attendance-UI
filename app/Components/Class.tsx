    ...
    <div className="w-full h-full flex flex-col rounded-3xl bg-zinc-900/60 backdrop-blur-2xl border border-emerald-500/30 p-[clamp(1rem,3vw,1.5rem)] shadow-2xl relative overflow-hidden">
      ...
      <div className="w-full flex p-1 rounded-full bg-zinc-800/40 border border-white/10 mb-4 shrink-0">
        {(["tahun", "bulan", "hari"] as const).map((tab) => (
          <button key={tab} onClick={() => setActiveTab(tab)} className={`flex-1 py-[clamp(0.4rem,2vw,0.6rem)] text-[clamp(0.75rem,2vw,0.875rem)] font-semibold capitalize rounded-full transition-all ${activeTab === tab ? "bg-white/20 text-white shadow-md" : "text-zinc-400"}`}>{tab}</button>
        ))}
      </div>
        ...
        <AnimatePresence mode="wait">
          {activeTab === "tahun" && (
            <motion.div key="tahun" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="grid grid-cols-3 sm:grid-cols-4 gap-[clamp(0.25rem,1.5vw,0.75rem)] p-1">
              {YEARS_LIST.map((year) => <button key={year} onClick={() => { setViewYear(year); setActiveTab("bulan"); }} className={`py-[clamp(0.5rem,2vw,0.75rem)] rounded-xl font-bold text-[clamp(0.875rem,2.5vw,1rem)] ${viewYear === year ? "bg-emerald-500/20 text-emerald-300 border-2 border-emerald-500" : "bg-zinc-800/50 text-zinc-400 hover:border-white/20 border border-transparent"}`}>{year}</button>)}
            </motion.div>
          )}
          {activeTab === "bulan" && (
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.1 }} className="w-full h-full">
      <Hover3DCard maxTilt={8} onClick={onClick} className="rounded-3xl bg-[#09090b]/80 backdrop-blur-xl border border-white/10 p-[clamp(1rem,3vw,1.5rem)] shadow-[inset_0_1px_1px_rgba(255,255,255,0.05),0_10px_20px_rgba(0,0,0,0.5)] flex flex-col gap-4 cursor-pointer hover:border-white/20 hover:shadow-2xl active:scale-95 active:bg-amber-300/10 h-full">
        <div className="relative z-10 flex flex-col gap-1 items-start">
          <h4 className="text-[clamp(1.5rem,4vw,2.25rem)] font-extrabold text-white tracking-tight drop-shadow-md">{group.className}</h4>
          <div className="flex items-center gap-2 mt-1">
            ...
            <span className="text-[clamp(0.6rem,1.5vw,0.75rem)] text-zinc-400 font-mono tracking-widest uppercase">{schedulesCount} Schedule(s)</span>
          </div>
        </div>
        ...
        <div className="relative z-10 flex-1 min-h-25 mt-4 p-4 rounded-2xl bg-zinc-950/50 border border-white/5 shadow-inner flex flex-wrap gap-2 content-start overflow-y-auto max-h-37.5 custom-scrollbar">
          {members.map((member) => (
            <div key={member.id} className="px-3 py-1.5 rounded-lg bg-zinc-800/80 border border-zinc-700/50 text-[clamp(0.7rem,1.5vw,0.8rem)] font-semibold text-zinc-200">{member.name}</div>
          ))}
          {members.length === 0 && <span className="text-[clamp(0.7rem,1.5vw,0.875rem)] text-zinc-600 italic m-auto">No students enrolled</span>}
        </div>
        <div className="relative z-10 text-right text-[clamp(0.7rem,1.5vw,0.75rem)] text-zinc-500 font-bold mt-2">Total: {members.length} Students</div>
      </Hover3DCard>
    </motion.div>
  );
}
  ...
  return (
    <div className="flex flex-col w-full h-full">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 pb-4 border-b border-zinc-800/80 gap-4">
        <h3 className="text-[clamp(1.4rem,5vw,2.4rem)] font-semibold text-white">Class Groups</h3>
        <button onClick={() => openModal({ modalType: "add_class", isEdit: false })} className="px-6 py-2.5 w-full sm:w-auto bg-emerald-600/20 hover:bg-emerald-600/40 text-emerald-300 hover:text-white rounded-xl text-[clamp(0.8rem,1.5vw,1rem)] font-semibold transition-all border border-emerald-500/30 hover:border-emerald-500/60">
          + Add New Class
        </button>
      </div>
