...
// ==========================================
// REUSABLE 3D GLARE CARD COMPONENT
// ==========================================
export function Hover3DCard({ children, className = "", maxTilt = 12, onClick }: { children: React.ReactNode; className?: string; maxTilt?: number; onClick?: () => void; }) {
  ...
  return (
    <div ref={cardRef} onMouseEnter={() => setIsHovering(true)} onMouseLeave={handleMouseLeave} onMouseMove={handleMouseMove} onClick={onClick} style={{ transform: transformStyle }} className={`relative overflow-hidden transition-all ${isHovering ? "duration-75 ease-out" : "duration-500 ease-out"} ${className}`}>
      {children}
...
export default function CardSiswa({ name, id, kelas, classSchedules, attendedData, onClick }: CardSiswaProps) {
  ...
  const dots = uniqueDays.map(date => {
     ...
     if (lateCount > 0) {
       // Jika semua jadwal yang di hari tsb terlambat (atau cuman 1 jadwal & terlambat) -> Merah
       if (lateCount === sessionsOnDay.length) return 'red';
       // Jika tercampur (ada yang hadir tepat waktu & ada yang terlambat / absen) -> Kuning
       return 'yellow'; 
     }
     // Sisanya: Tidak ada keterlambatan, tapi ada absen -> Kuning
     if (onTimeCount > 0 && onTimeCount < sessionsOnDay.length) return 'yellow'; 
     
     // Full presensi sempurna
     return 'green'; 
  });

  // Kalkulasi persentase, TIDAK memasukkan yg terlambat (hanya hitung yg tidak ada status 'terlambat')
        ...
        <div className="relative z-10 flex flex-col gap-2">
          ...
          <div className="mt-2 flex gap-1.5 flex-wrap">
            {dots.length > 0 ? dots.map((status, idx) => (
              ...
              <span className="text-[10px] text-zinc-600 italic">No schedules set</span>
            )}
          </div>
          ...
          <div className="mt-4 flex flex-col gap-2.5">
            ...
            <div className="flex justify-between items-center px-1">
              <span className="text-[10px] text-zinc-400 font-bold uppercase tracking-[0.25em]">Attendance Rate</span>
              <span className="text-base font-extrabold text-emerald-400 drop-shadow-[0_0_12px_rgba(52,211,153,1)] tracking-widest">{percentage}%</span>
            </div>
          </div>
        </div>
      </Hover3DCard>
      ...
      <Hover3DCard maxTilt={12} onClick={onClick} className="rounded-2xl bg-[#09090b]/80 border border-white/10 p-5 cursor-pointer shadow-[inset_0_1px_1px_rgba(255,255,255,0.05),0_10px_20px_rgba(0,0,0,0.5)] active:scale-95 active:bg-amber-300/50 duration-100">
        <div className="relative z-10 flex flex-col gap-3">
          ...
          <div className="mt-3 flex gap-4">
            ...
            {attendanceTimes.length === 0 && <span className="text-xs text-zinc-600 italic">Belum ada presensi</span>}
          </div>
        </div>
      </Hover3DCard>
    </motion.div>
